...
assert.doesNotMatch(api, /product\.stockQuantity\s*>\s*0/, "the portal must show products with zero stock too");
assert.match(app, /כמות לא הוגדרה/);
assert.match(app, /out-stock/);
...
assert.match(app, /syncOrderInBackground/);
assert.match(app, /\?action=sync-order/);
assert.match(api, /ORDER_SYNC_MAX_ATTEMPTS = 3/);
assert.match(api, /ORDER_SYNC_RETRY_BATCH_SIZE = 2/);
assert.match(api, /sync_attempts < \$\{ORDER_SYNC_MAX_ATTEMPTS\}/);
assert.match(api, /next_sync_at <= now\(\)/);
assert.match(api, /syncSavedOrderOnce\(sql, config, created\)/);
assert.match(api, /syncSavedOrderOnce\(sql, config, updated\)/);
assert.match(api, /syncSavedOrderOnce\(sql, config, pendingDelete\)/);
assert.doesNotMatch(api, /while\s*\(/, "order synchronization must never use an unbounded loop");
const addToCart = app.slice(app.indexOf("function addPendingToCart"), app.indexOf("function renderCart"));
assert.doesNotMatch(addToCart, /showActionToast/, "adding a product must not show a temporary cart toast");
