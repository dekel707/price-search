...
assert.match(mainApp, /function isEitanPartnerOrder/);
assert.match(mainApp, /order-source-badge eitan/);
assert.doesNotMatch(mainHtml, /data-tab="eitan-orders"/, "Eitan orders must appear in the relevant order tabs, not in a separate owner tab.");
assert.doesNotMatch(mainHtml, /הזמנות שאיתן שלח היום נכנסות מיד/, "The owner-only Eitan queue must not remain visible in the main portal.");
