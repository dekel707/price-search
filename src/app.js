...
const dom = {
  ...
  scheduledReminderEmailPreview: document.querySelector("#scheduledReminderEmailPreview"),
  scheduledReminderEmailPreviewCard: document.querySelector("#scheduledReminderEmailPreviewCard"),
  scheduledReminderEmailPreviewRecipient: document.querySelector("#scheduledReminderEmailPreviewRecipient"),
  scheduledReminderEmailPreviewType: document.querySelector("#scheduledReminderEmailPreviewType"),
  scheduledReminderEmailPreviewPriority: document.querySelector("#scheduledReminderEmailPreviewPriority"),
  scheduledReminderEmailPreviewSubject: document.querySelector("#scheduledReminderEmailPreviewSubject"),
  scheduledReminderEmailPreviewMessage: document.querySelector("#scheduledReminderEmailPreviewMessage"),
  scheduledReminderEmailPreviewDue: document.querySelector("#scheduledReminderEmailPreviewDue"),
  scheduledReminderEmailPreviewRecurrence: document.querySelector("#scheduledReminderEmailPreviewRecurrence"),
...
function bindEvents() {
  ...
  dom.results.addEventListener("click", (event) => {
    ...
    const futureStockButton = event.target.closest("[data-add-future-stock]");
  ...
  const handleCartFieldInput = (event) => {
    const bundleQuantityInput = event.target.closest("[data-cart-bundle-quantity]");
    if (bundleQuantityInput) {
      updateBundleCartQuantity(
        bundleQuantityInput.dataset.cartBundleQuantity,
        parseQuantity(bundleQuantityInput.value),
        { render: false },
      );
      return;
    }
  ...
  const handleCartFieldChange = (event) => {
    const bundleQuantityInput = event.target.closest("[data-cart-bundle-quantity]");
    if (bundleQuantityInput) {
      updateBundleCartQuantity(bundleQuantityInput.dataset.cartBundleQuantity, parseQuantity(bundleQuantityInput.value));
      return;
    }
...
function renderResultNodes(items, query, totalMatches) {
  ...
  return items.map((product) => {
    ...
    if (futureStockEligible) {
      const futureStockButton = document.createElement("button");
      futureStockButton.className = "future-stock-button";
      futureStockButton.type = "button";
      futureStockButton.dataset.addFutureStock = product.skuKey;
...
function getFutureStockEligibleCartProducts() {
  const seen = new Set();
  return cart
    .map((line) => products.find((product) => product.skuKey === line.skuKey))
    .filter((product) => product && isFutureStockEligibleProduct(product))
    .filter((product) => {
      if (seen.has(product.skuKey)) return false;
      seen.add(product.skuKey);
      return true;
    });
}
...
function renderScheduledReminderEmailPreview() {
  if (!dom.scheduledReminderEmailPreview) return;
  const title = cleanString(dom.scheduledReminderTitle.value) || "כותרת התזכורת תופיע כאן";
  const message = cleanString(dom.scheduledReminderMessage.value) || "פירוט התזכורת יופיע כאן.";
  const recipient = cleanString(scheduledEmailReminderConfig.defaultRecipient || dom.scheduledReminderRecipient.value);
  const dueDate = normalizeDateInput(dom.scheduledReminderDate.value);
  const dueTime = cleanString(dom.scheduledReminderTime.value);
  const priority = normalizeScheduledReminderPriority(dom.scheduledReminderPriority.value);
  const type = normalizeScheduledReminderType(dom.scheduledReminderType.value);
  const recurrence = normalizeScheduledReminderRecurrence(dom.scheduledReminderRecurrence.value);

  dom.scheduledReminderEmailPreviewRecipient.textContent = recipient ? `אל: ${recipient}` : "נמען יופיע כאן";
  dom.scheduledReminderEmailPreviewType.textContent = scheduledReminderTypeLabel(type);
  dom.scheduledReminderEmailPreviewPriority.textContent = `עדיפות ${scheduledReminderPriorityLabel(priority)}`;
  dom.scheduledReminderEmailPreviewSubject.textContent = title;
  dom.scheduledReminderEmailPreviewMessage.textContent = message;
  dom.scheduledReminderEmailPreviewDue.textContent = dueDate
    ? `מועד: ${formatReminderDate(dueDate)}${dueTime ? ` · ${dueTime}` : ""}`
    : "בחר תאריך ושעה";
...
function getYearOverYearReports() {
  ...
  if (!config) return { config: null, reports: [], pace: null };
  const pace = getYearOverYearPace(config);
  ...
  const reports = [...reportsByKey.values()]
    .map((report) => {
      ...
      const paceGapPercent = hasTarget ? roundMoney((attainment || 0) - pace.elapsedPercent) : null;
      const projectedSales = pace.daysElapsed > 0 ? roundMoney((sales2026 / pace.daysElapsed) * pace.daysInYear) : sales2026;
      const projectedAttainment = hasTarget ? (projectedSales / report.sales2025) * 100 : null;
      const remaining = hasTarget ? Math.max(0, roundMoney(report.sales2025 - sales2026)) : 0;
      const requiredMonthlySales =
        remaining > 0 && pace.daysRemaining > 0 ? roundMoney(remaining / (pace.daysRemaining / 30.4375)) : 0;
      ...
      return { ...enrichedReport, signal: getYearOverYearSignal(enrichedReport) };
    })
    .sort(
      (a, b) =>
...
function getYearOverYearPace(config) {
  const cutoff = normalizeDateInput(config?.baselineCutoffDate) || "2026-07-31";
  const year = Number(cutoff.slice(0, 4)) || new Date().getFullYear();
  const yearStart = `${year}-01-01`;
  const yearEnd = `${year}-12-31`;
  const today = getIsraelDateKey(new Date());
  const toDate = today < yearStart ? yearStart : today > yearEnd ? yearEnd : today;
  const daysInYear = getYearOverYearDayDifference(yearStart, yearEnd) + 1;
  const daysElapsed = getYearOverYearDayDifference(yearStart, toDate) + 1;
  ...
  return {
    ...
    daysElapsed,
    daysRemaining,
    elapsedPercent: (daysElapsed / daysInYear) * 100,
  };
}

function getYearOverYearDayDifference(fromDateKey, toDateKey) {
...
function getYearOverYearSignal(report) {
  const materialGap = Math.max(Math.abs(report.paceGapAmount || 0), report.remaining || 0);
  const isMaterial = report.sales2025 >= 25_000 || materialGap >= 20_000;
  ...
  if (report.sales2026 <= 0 && report.sales2025 >= 25_000) {
    return { tone: "danger", label: "ללא מכר השנה", action: "כדאי לבדוק קשר ומכר חוזר" };
  }
  if (isMaterial && (report.paceGapPercent || 0) <= -15) {
    return { tone: "danger", label: "מתחת לקצב", action: `נדרש קצב של ${formatPrice(report.requiredMonthlySales)} לחודש` };
  }
  if (isMaterial && (report.paceGapPercent || 0) <= -6) {
    return { tone: "warning", label: "דורש מעקב", action: `פער לקצב: ${formatPrice(Math.abs(report.paceGapAmount))}` };
  }
  if (report.passed && isMaterial) {
    return { tone: "positive", label: "עבר את 2025", action: "לשמר את הקצב ולהרחיב מכר" };
  }
  if (isMaterial && (report.paceGapPercent || 0) >= 10) {
    return { tone: "positive", label: "מעל הקצב", action: "קצב שנתי צפוי לעבור את 2025" };
  }
  return { tone: "neutral", label: "בקצב יציב", action: "המשך מעקב לפי הקצב החודשי" };
}
...
function renderYearOverYearInsights(reports, pace) {
  const insights = getYearOverYearInsightRows(reports);
  if (!insights.length) {
    dom.yearOverYearInsights.replaceChildren(emptyState("אין כרגע חריגות משמעותיות לזיהוי."));
    return;
  }
...
function getYearOverYearInsightRows(reports) {
  ...
  const attention = reports
    .filter((report) => ["danger", "warning"].includes(report.signal.tone))
    .sort(
      (left, right) =>
        getYearOverYearAttentionRank(right) - getYearOverYearAttentionRank(left) ||
        right.remaining - left.remaining,
    )
    .slice(0, 4);
  const positive = reports
    .filter((report) => report.signal.tone === "positive")
    .sort(
      (left, right) =>
        (right.projectedAttainment || 0) - (left.projectedAttainment || 0) ||
        right.sales2026 - left.sales2026,
    )
    .slice(0, 4);
  const selected = [...attention, ...positive];
  const seen = new Set();
  return selected.filter((report) => {
    if (seen.has(report.key)) return false;
    seen.add(report.key);
    return true;
  });
}

function getYearOverYearAttentionRank(report) {
  const toneRank = report.signal.tone === "danger" ? 2 : report.signal.tone === "warning" ? 1 : 0;
  return toneRank * 10_000_000 + Math.max(report.remaining || 0, Math.abs(report.paceGapAmount || 0));
}

function renderYearOverYearInsightRow(report, pace) {
  const row = document.createElement("article");
  row.className = `year-over-year-insight-row is-${report.signal.tone}`;
  const forecast = report.hasTarget ? formatYearOverYearPercent(report.projectedAttainment) : "חדש";
  const paceValue = report.hasTarget
    ? `${report.paceGapPercent >= 0 ? "+" : ""}${formatYearOverYearPercent(report.paceGapPercent)}`
    : "—";
  const gapDescription = report.hasTarget
    ? report.paceGapAmount >= 0
      ? `מעל קצב השנה ב־${formatPrice(report.paceGapAmount)}`
      : `מתחת לקצב השנה ב־${formatPrice(Math.abs(report.paceGapAmount))}`
    : "אין בסיס 2025 להשוואה";

  row.innerHTML = `
    <div class="year-over-year-insight-customer">
      <strong>${escapeHtml(report.customerName)}</strong>
      <span>${escapeHtml(report.customerCode ? `קוד ${report.customerCode}` : "ללא קוד לקוח")}</span>
    </div>
    <span class="year-over-year-signal is-${escapeHtml(report.signal.tone)}">${escapeHtml(report.signal.label)}</span>
    <div class="year-over-year-insight-value">
      <span>פער מהקצב</span>
      <strong>${escapeHtml(paceValue)}</strong>
      <small>${escapeHtml(gapDescription)}</small>
    </div>
    <div class="year-over-year-insight-value">
      <span>תחזית סוף ${pace.year}</span>
      <strong>${escapeHtml(forecast)}</strong>
      <small>${escapeHtml(report.hasTarget ? formatPrice(report.projectedSales) : report.signal.action)}</small>
    </div>
    <div class="year-over-year-insight-action">
      <span>מה לשים לב</span>
      <strong>${escapeHtml(report.signal.action)}</strong>
    </div>
  `;
  return row;
}
...
function renderYearOverYearCard(report, openDetails) {
  ...
  const sourceStatus = report.sourceOnly
    ...
    : report.currentOrderCount
      ...
      : "מחובר ללקוח במערכת";
  const paceDescription = report.hasTarget
    ? report.paceGapAmount >= 0
      ? `מעל קצב השנה ב־${formatPrice(report.paceGapAmount)}`
      : `מתחת לקצב השנה ב־${formatPrice(Math.abs(report.paceGapAmount))}`
    : "ללא בסיס 2025 להשוואה";
  ...
  summary.innerHTML = `
    ...
    <div class="year-over-year-kpi">
      ...
      <b class="year-over-year-signal is-${escapeHtml(report.signal.tone)}">${escapeHtml(report.signal.label)}</b>
    </div>
  `;
  ...
  details.innerHTML = `
    ...
    <p>${escapeHtml(`${comparison} · ${paceDescription}`)}</p>
  `;
...
function formatYearOverYearDate(dateKey) {
  ...
  return Number.isNaN(date.getTime()) ? dateKey : date.toLocaleDateString("he-IL", { day: "2-digit", month: "2-digit" });
}
...
function isFutureStockDraft(draft) {
  return Boolean(draft?.futureStockOrder && normalizeDateInput(draft.futureStockDate));
}

function getFutureStockReminderId(draftId) {
  return `future-stock-reminder-${draftId}`;
}

function buildFutureStockReminderTitle(draft) {
  const customer = getOrderCustomer(draft);
  const customerName = customer?.name || draft.customerName || "ללא לקוח";
  const itemSummary = draft.items
    ...
    .join(" · ");
  return `מלאי עתידי: ${customerName}${itemSummary ? ` · ${itemSummary}` : ""}`;
}

function syncFutureStockReminder(draft) {
  const reminderId = getFutureStockReminderId(draft.id);
  const existing = reminders.find((reminder) => reminder.id === reminderId);
  const shouldCreate = isFutureStockDraft(draft);
  const customer = getOrderCustomer(draft);
  const now = new Date().toISOString();
  const nextReminder = shouldCreate
    ? {
        id: reminderId,
        title: buildFutureStockReminderTitle(draft),
        dueDate: draft.futureStockDate,
        ...
        createdAt: existing?.createdAt || now,
        updatedAt: now,
        sourceType: "future-stock",
        sourceId: draft.id,
      }
    : null;

  const unchanged =
    Boolean(existing && nextReminder) &&
    existing.title === nextReminder.title &&
    existing.dueDate === nextReminder.dueDate &&
    existing.customerId === nextReminder.customerId &&
    existing.customerName === nextReminder.customerName &&
    !existing.completed;
  if (unchanged) return false;

  reminders = reminders.filter((reminder) => reminder.id !== reminderId);
  if (nextReminder) reminders = [nextReminder, ...reminders];
  if (existing || nextReminder) saveReminders({ sync: false });
  return Boolean(existing || nextReminder);
}

function removeFutureStockReminder(draftId) {
  const reminderId = getFutureStockReminderId(draftId);
...
function getOrderDestinationTab(order, reference = new Date()) {
  if (isOrderCompleted(order)) return "completed-orders";
  return isOrderForTomorrow(order, reference) ? "tomorrow-orders" : "orders";
}
...
function upsertCartLine(product, quantity, options) {
  ...
  if (existing) {
    ...
    existing.promotionType = normalizePromotionType(options.promotionType);
    return;
  }
  ...
  cart.push({
    ...
    promotionType: normalizePromotionType(options.promotionType),
    quantity,
    fromReservation: Boolean(options.fromReservation),
  });
}

function createCartLineKey(skuKey, fromReservation, unitPrice = 0, priceSource = "list", bonusType = "", promotionId = "", promotionType = "unit") {
  ...
  const promotionKey = normalizePriceSource(priceSource) === "promotion"
    ? `${cleanString(promotionId) || "promotion"}::${normalizePromotionType(promotionType)}`
    : "";
...
function mergeCartLines(lines) {
  ...
  lines.forEach((line) => {
    ...
    if (existing) {
      ...
      if (isBundlePromotionCartLine(existing)) {
        existing.promotionBundleQuantity = parseQuantity(existing.promotionBundleQuantity) + parseQuantity(line.promotionBundleQuantity);
        existing.promotionBundleLineTotal = roundMoney(
          (parsePrice(existing.promotionBundleLineTotal) ?? 0) + (parsePrice(line.promotionBundleLineTotal) ?? 0),
        );
        existing.unitPrice = roundMoney(existing.promotionBundleLineTotal / Math.max(1, existing.quantity));
      }
    } else {
...
function updateBundleCartQuantity(lineKey, quantity, options = { render: true }) {
  const sourceLine = cart.find((line) => line.lineKey === lineKey);
  if (!sourceLine || !isBundlePromotionCartLine(sourceLine)) return;
  const bundleId = sourceLine.promotionBundleId;
  const nextBundleQuantity = parseQuantity(quantity);
  const updatedLines = cart.map((line) => {
    if (!isBundlePromotionCartLine(line) || line.promotionBundleId !== bundleId) return line;
    const itemQuantity = parseQuantity(line.promotionBundleItemQuantity);
    const bundleUnitTotal = Math.max(0, parsePrice(line.promotionBundleUnitTotal) ?? 0);
    const nextQuantity = nextBundleQuantity * itemQuantity;
    const nextLineTotal = roundMoney(bundleUnitTotal * nextBundleQuantity);
    return {
      ...line,
      quantity: nextQuantity,
      promotionBundleQuantity: nextBundleQuantity,
      promotionBundleLineTotal: nextLineTotal,
      unitPrice: roundMoney(nextLineTotal / Math.max(1, nextQuantity)),
    };
  });
  cart = orderCartLines(updatedLines);
  saveCart();
  if (options.render === false) renderCartSummary();
  else renderCart();
}
...
function removeCartLine(lineKey) {
  ...
  const isBundle = isBundlePromotionCartLine(removedLine);
  cart = isBundle
    ? cart.filter((line) => !isBundlePromotionCartLine(line) || line.promotionBundleId !== removedLine.promotionBundleId)
    : cart.filter((line) => line.lineKey !== lineKey);
  if (!cart.length) {
    editingOrderId = "";
    editingDraftId = "";
    duplicatedOrderNeedsCustomer = false;
  ...
  announceDeletion(isBundle
    ? `הבאנדל „${removedLine.promotionName || "מבצע"}“ הוסר מהסל.`
    : `${removedLine.description || removedLine.sku || "המוצר"} הוסר מהסל.`);
}
...
function saveDraftOrder(options = {}) {
  ...
  const draft = {
    ...
    orderType,
    futureStockOrder,
    futureStockDate: futureStockOrder ? futureStockDate : "",
  ...
  dom.status.textContent =
    options.status ||
    (futureStockOrder
      ? `הזמנת המלאי העתידי נשמרה. תזכורת תופיע ב־${formatReminderDate(futureStockDate)} והיא לא נכנסה להזמנות או לדוחות.`
      : originalDraft
...
function normalizePromotionType(value) {
  return value === "bundle" ? "bundle" : "unit";
}

function isBundlePromotion(promotion) {
  return normalizePromotionType(promotion?.type) === "bundle";
}
...
function createPromotionDraft(promotion) {
  ...
  return {
    ...
    bundlePrice: Math.max(0, parsePrice(promotion.bundlePrice) ?? 0),
...
function getPromotionTotal(promotion) {
  if (isBundlePromotion(promotion)) return roundMoney(Math.max(0, parsePrice(promotion.bundlePrice) ?? 0));
  return roundMoney(
...
function getPromotionTotalLabel(promotion) {
  return `${isBundlePromotion(promotion) ? "מחיר חבילה" : "סה״כ פריטים"} ${formatPrice(getPromotionTotal(promotion))}`;
}
...
function renderPromotionBuilder() {
  ...
  total.textContent = getPromotionTotalLabel(promotionDraft);
  ...
  const typeField = createPromotionTypeField(promotionDraft.type);
  const bundlePriceField = isBundlePromotion(promotionDraft)
    ? createPromotionStandaloneNumberField("מחיר מיוחד לכל החבילה", "bundlePrice", promotionDraft.bundlePrice, 0, 0.01)
    : null;
  ...
  fields.append(nameField, descriptionField, typeField, ...(bundlePriceField ? [bundlePriceField] : []), activeLabel);
  ...
  itemsTitle.textContent = isBundlePromotion(promotionDraft) ? "פריטי החבילה" : "פריטי המבצע";
  ...
  if (!promotionDraft.items.length) {
    ...
    hint.textContent = isBundlePromotion(promotionDraft)
      ? "הוסף את המוצרים וכמות כל מוצר בכל חבילה. המחיר למעלה חל על החבילה כולה."
      : "הוסף דגם, כמות ומחיר מבצע לכל פריט.";
    items.append(hint);
  } else {
...
function createPromotionTypeField(value) {
  const label = document.createElement("label");
  label.className = "field-wrap promotion-type-field";
  const text = document.createElement("span");
  text.textContent = "סוג מבצע";
  const select = document.createElement("select");
  select.dataset.promotionField = "type";
  [
    ["unit", "מחיר לכל מוצר"],
    ["bundle", "באנדל · מחיר לחבילה"],
  ].forEach(([optionValue, labelText]) => {
    const option = document.createElement("option");
    option.value = optionValue;
    option.textContent = labelText;
    option.selected = normalizePromotionType(value) === optionValue;
...
function createPromotionStandaloneNumberField(labelText, field, value, min, step) {
  const label = document.createElement("label");
  label.className = "field-wrap promotion-bundle-price-field";
  const text = document.createElement("span");
  text.textContent = labelText;
  const input = document.createElement("input");
  input.type = "number";
...
function renderPromotionBuilderItem(item, index) {
  ...
  row.classList.toggle("bundle-promotion-builder-item", isBundlePromotion(promotionDraft));
  ...
  const quantity = createPromotionNumberField(
    isBundlePromotion(promotionDraft) ? "כמות בכל חבילה" : "כמות",
  ...
  const price = isBundlePromotion(promotionDraft)
    ? null
    : createPromotionNumberField("מחיר מבצע ליח׳", "promotionItemPrice", index, item.unitPrice, 0, 0.01);
  const remove = document.createElement("button");
  remove.type = "button";
  ...
  top.append(productField, quantity, ...(price ? [price] : []), remove);

  const details = document.createElement("p");
  details.className = "promotion-builder-item-details";
  details.textContent = item.skuKey
    ? isBundlePromotion(promotionDraft)
      ? `${item.description || "ללא תיאור"} · ${parseQuantity(item.quantity).toLocaleString("he-IL")} יח׳ בכל חבילה`
...
function handlePromotionBuilderInput(event) {
  ...
  if (field) {
    if (field === "type") {
      promotionDraft.type = normalizePromotionType(input.value);
      renderPromotionsPanel();
      return;
    }
    if (field === "bundlePrice") {
      promotionDraft.bundlePrice = Math.max(0, parsePrice(input.value) ?? 0);
...
function handlePromotionBuilderChange(event) {
  ...
  if (event.target.dataset.promotionField === "type") {
    promotionDraft.type = normalizePromotionType(event.target.value);
    renderPromotionsPanel();
  }
}
...
function updatePromotionBuilderTotal() {
  ...
  if (total) total.textContent = getPromotionTotalLabel(promotionDraft);
}
...
function savePromotionFromBuilder() {
  ...
  const type = normalizePromotionType(promotionDraft.type);
  const items = normalizePromotionItems(promotionDraft.items);
  const parsedBundlePrice = parsePrice(promotionDraft.bundlePrice);
  const bundlePrice = Math.max(0, parsedBundlePrice ?? 0);
  if (!name) {
  ...
  if (!items.length) {
    dom.status.textContent = "יש להוסיף לפחות מוצר אחד למבצע.";
    return;
  }
  if (type === "bundle" && (parsedBundlePrice === null || parsedBundlePrice <= 0)) {
    dom.status.textContent = "יש להגדיר מחיר חיובי לחבילה.";
    return;
  }
  const now = new Date().toISOString();
  const promotion = {
    ...
    type,
    bundlePrice: type === "bundle" ? bundlePrice : 0,
...
function addPromotionToCart(promotionId) {
  ...
  if (isBundlePromotion(promotion)) {
    addBundlePromotionToCart(promotion);
    return;
  }
...
function isBundlePromotionCartLine(line) {
  return line?.priceSource === "promotion"
    && line?.promotionType === "bundle"
    && Boolean(cleanString(line.promotionBundleId || line.promotionId));
}

function getBundleLineShares(promotion) {
  const groupedItems = new Map();
  (promotion.items || []).forEach((item) => {
    const skuKey = getSkuKey(item.skuKey || item.sku);
    if (!skuKey) return;
    const existing = groupedItems.get(skuKey);
    if (existing) {
      existing.quantity = parseQuantity(existing.quantity) + parseQuantity(item.quantity);
      return;
    }
    groupedItems.set(skuKey, { ...item, skuKey, quantity: parseQuantity(item.quantity) });
  });
  const items = [...groupedItems.values()].map((item) => ({
    item,
    quantity: parseQuantity(item.quantity),
    weight: Math.max(0, Number(item.listPrice) || 0) * parseQuantity(item.quantity),
  }));
  const fallbackWeight = items.reduce((sum, entry) => sum + entry.quantity, 0) || 1;
  const weightedTotal = items.reduce((sum, entry) => sum + entry.weight, 0);
  const totalWeight = weightedTotal || fallbackWeight;
  const weightedItems = items.map((entry) => ({ ...entry, allocationWeight: weightedTotal ? entry.weight : entry.quantity }));
  const lastPayingIndex = weightedItems.reduce((last, entry, index) => (entry.allocationWeight > 0 ? index : last), weightedItems.length - 1);
  let remainingCents = Math.round(getPromotionTotal(promotion) * 100);
  let remainingWeight = totalWeight;
  return weightedItems.map((entry, index) => {
    const weight = entry.allocationWeight;
    const cents = index === lastPayingIndex
      ? remainingCents
      : weight > 0
        ? Math.max(0, Math.min(remainingCents, Math.round(remainingCents * (weight / Math.max(1, remainingWeight)))))
        : 0;
    remainingCents -= cents;
    remainingWeight -= weight;
    return { ...entry, lineTotal: roundMoney(cents / 100) };
  });
}

function addBundlePromotionToCart(promotion) {
  const shares = getBundleLineShares(promotion);
  const prepared = shares.map(({ item }) => ({ item, product: products.find((candidate) => candidate.skuKey === item.skuKey) }));
  const missing = prepared.filter(({ product }) => !product).map(({ item }) => item.sku || item.skuKey);
  if (missing.length) {
    dom.status.textContent = `החבילה לא נוספה: ${missing.join(", ")} אינם נמצאים במחירון הנוכחי.`;
    showActionToast(dom.status.textContent, "error");
    return;
  }

  let addedQuantity = 0;
  prepared.forEach(({ item, product }, index) => {
    const share = shares[index];
    const perBundleQuantity = parseQuantity(item.quantity);
    const existing = cart.find((line) => isBundlePromotionCartLine(line)
      && line.promotionBundleId === promotion.id
      && line.skuKey === product.skuKey);
    if (existing) {
      const nextBundleQuantity = parseQuantity(existing.promotionBundleQuantity) + 1;
      const bundleUnitTotal = Math.max(0, parsePrice(existing.promotionBundleUnitTotal) ?? share.lineTotal);
      const nextQuantity = nextBundleQuantity * parseQuantity(existing.promotionBundleItemQuantity || perBundleQuantity);
      existing.promotionBundleQuantity = nextBundleQuantity;
      existing.quantity = nextQuantity;
      existing.promotionBundleLineTotal = roundMoney(bundleUnitTotal * nextBundleQuantity);
      existing.unitPrice = roundMoney(existing.promotionBundleLineTotal / Math.max(1, nextQuantity));
    } else {
      const lineKey = createCartLineKey(product.skuKey, false, roundMoney(share.lineTotal / Math.max(1, perBundleQuantity)), "promotion", "", promotion.id, "bundle");
      cart.push({
        lineKey,
        skuKey: product.skuKey,
        sku: product.sku,
        description: product.description,
        listPrice: product.price,
        unitPrice: roundMoney(share.lineTotal / Math.max(1, perBundleQuantity)),
        priceSource: "promotion",
        bonusType: "",
        promotionId: promotion.id,
        promotionName: promotion.name,
        promotionType: "bundle",
        promotionBundleId: promotion.id,
        promotionBundleQuantity: 1,
        promotionBundleItemQuantity: perBundleQuantity,
        promotionBundlePrice: getPromotionTotal(promotion),
        promotionBundleUnitTotal: share.lineTotal,
        promotionBundleLineTotal: share.lineTotal,
        quantity: perBundleQuantity,
        fromReservation: false,
      });
    }
    addedQuantity += perBundleQuantity;
  });
  cart = orderCartLines(cart);
  saveCart();
  render();
  setActiveTab("cart");
  const message = `התווספה חבילה אחת לסל: ${promotion.name} · ${addedQuantity.toLocaleString("he-IL")} פריטים · ${formatPrice(getPromotionTotal(promotion))}.`;
  dom.status.textContent = message;
  showActionToast(message);
}
...
function renderPromotionCard(promotion) {
  ...
  const promotionKind = isBundlePromotion(promotion) ? "באנדל · מחיר לחבילה" : "מחיר נפרד לכל מוצר";
  meta.textContent = promotion.active ? `${promotionKind} · מוכן לסל` : `${promotionKind} · מבצע מושבת`;
  copy.append(title, meta);
  const total = document.createElement("strong");
  total.className = "promotion-card-total";
  total.textContent = getPromotionTotalLabel(promotion);
  ...
  promotion.items.forEach((item) => {
    ...
    const detail = document.createElement("span");
    detail.textContent = isBundlePromotion(promotion)
      ? `${item.description || "ללא תיאור"} · ${parseQuantity(item.quantity).toLocaleString("he-IL")} יח׳ בכל חבילה`
      : `${item.description || "ללא תיאור"} · ${formatPrice(item.unitPrice)} ליח׳`;
...
function renderCartLine(line) {
  ...
  const bundleLine = isBundlePromotionCartLine(line);
  const row = document.createElement("article");
  ...
  row.classList.toggle("bundle-promotion-cart-line", bundleLine);
  ...
  const quantity = createNumberField(bundleLine ? "כמות חבילות" : "כמות", bundleLine ? line.promotionBundleQuantity : line.quantity, {
    min: 1,
    step: 1,
    attr: bundleLine ? "cartBundleQuantity" : "cartQuantity",
    key: line.lineKey,
  });
  const price = bundleLine
    ? createBundlePromotionPriceField(line)
    : line.fromReservation
  ...
  if (!bundleLine && !line.fromReservation && !isBonusOrderItem(line)) {
    const listButton = document.createElement("button");
    listButton.type = "button";
  ...
  promotionBadge.textContent = bundleLine
    ? `באנדל${line.promotionName ? ` · ${line.promotionName}` : ""} · ${parseQuantity(line.promotionBundleQuantity).toLocaleString("he-IL")} חבילות · ${formatPrice(line.promotionBundlePrice)} לחבילה`
    : `מחיר מבצע${line.promotionName ? ` · ${line.promotionName}` : ""}`;
...
function createBundlePromotionPriceField(line) {
  const field = document.createElement("div");
  field.className = "field-wrap compact-field bundle-promotion-price-field";
  field.innerHTML = `<span>מחיר החבילה</span><strong>${escapeHtml(formatPrice(line.promotionBundlePrice))}</strong>`;
  return field;
}
...
function renderEitanOrdersPanel() {
  ...
  dom.eitanOrdersStatus.textContent = eitanPortalOrders.length
    ? `${eitanPortalOrders.length.toLocaleString("he-IL")} הזמנות איתן שנשלחו היום`
    : "אין הזמנות איתן שנשלחו היום.";
...
function renderEitanOrderCard(order) {
  ...
  const approved = order.status === "approved" || order.status === "sent_to_main";
  card.innerHTML = `
    <div class="order-body">
      <div class="order-card-heading">
        ...
        <span class="eitan-order-state ${approved ? "approved" : "pending"}">${approved ? "נשלחה למערכת" : "נשלחה"}</span>
      </div>
    ...
    <div class="order-actions"><button class="secondary-button" type="button" data-toggle-eitan-details="${escapeHtml(order.id)}">הצג הזמנה</button></div>
...
function renderDrafts() {
  ...
  const ordinaryDrafts = drafts.filter((draft) => !isFutureStockDraft(draft));
  const visibleDrafts = ordinaryDrafts.filter((draft) => {
    if (!query) return true;
    const searchable = [
      draft.customerName,
      draft.customerCode,
  ...
  dom.draftsSummary.textContent = query
    ? `${visibleDrafts.length.toLocaleString("he-IL")} מתוך ${ordinaryDrafts.length.toLocaleString("he-IL")}`
    : ordinaryDrafts.length === 1
      ? "טיוטה אחת"
      : `${ordinaryDrafts.length.toLocaleString("he-IL")} טיוטות`;
  if (!ordinaryDrafts.length) {
...
function renderFutureStockOrders() {
  const query = normalizeSearch(dom.futureStockOrdersSearch.value);
  const futureStockOrders = drafts
    .filter(isFutureStockDraft)
    .filter((draft) => {
      if (!query) return true;
      const searchable = [
    ...
    .sort((a, b) => a.futureStockDate.localeCompare(b.futureStockDate) || compareOrdersByCreatedAt(a, b));

  const allFutureStockOrders = drafts.filter(isFutureStockDraft);
  dom.futureStockOrdersSummary.textContent = query
    ? `${futureStockOrders.length.toLocaleString("he-IL")} מתוך ${allFutureStockOrders.length.toLocaleString("he-IL")}`
    : allFutureStockOrders.length === 1
      ? "הזמנה אחת"
      : `${allFutureStockOrders.length.toLocaleString("he-IL")} הזמנות`;

  if (!allFutureStockOrders.length) {
    dom.futureStockOrdersList.replaceChildren(emptyState("אין הזמנות שממתינות למלאי עתידי."));
    return;
  }
  if (!futureStockOrders.length) {
    dom.futureStockOrdersList.replaceChildren(emptyState("לא נמצאו הזמנות מלאי עתידי מתאימות."));
    return;
  }
...
function renderFutureStockOrderCard(draft) {
  const card = document.createElement("article");
  card.className = "order-card draft-card future-stock-order-card";
  const customer = getOrderCustomer(draft);
  ...
  const summary = draft.items
    ...
    .map((item) => `${item.description} × ${item.quantity}`)
    .join(" · ");
  const isDue = draft.futureStockDate <= getLocalDateKey(new Date());

  const body = document.createElement("div");
  body.className = "order-body";
  body.innerHTML = `
    <div class="order-card-heading">
      <span class="order-card-kind-icon calendar" aria-hidden="true">${getOrderActionIcon("calendar")}</span>
      <div class="order-card-heading-copy">
        <strong>${escapeHtml(formatReminderDate(draft.futureStockDate))}</strong>
        <span class="order-card-state">מלאי עתידי</span>
      </div>
  ...
  actions.innerHTML = `
    ...
    <a class="whatsapp-button compact order-action-button order-action-whatsapp" href="${escapeHtml(createWhatsAppUrl(draft.items, draft) || "#")}" target="_blank" rel="noreferrer" data-send-future-stock="${escapeHtml(draft.id)}">${getOrderActionIcon("whatsapp")}<span>וואטסאפ</span></a>
    <button class="danger-button order-action-button order-action-delete" type="button" data-delete-future-stock="${escapeHtml(draft.id)}">${getOrderActionIcon("trash")}<span>מחק</span></button>
  `;
  const whatsapp = actions.querySelector("[data-send-future-stock]");
  if (!createWhatsAppUrl(draft.items, draft)) {
  ...
  date.className = "order-details-meta";
  date.textContent = `תאריך תזכורת למלאי: ${formatReminderDate(draft.futureStockDate)}`;
  details.prepend(date);
...
function formatOrderLine(item) {
  ...
  if (isBundlePromotionCartLine(item)) {
    return `${prefix} · באנדל${item.promotionName ? ` ${item.promotionName}` : ""}`;
  }
...
function handleFutureStockOrderActionClick(event) {
  ...
  const editButton = event.target.closest("[data-edit-future-stock]");
  if (editButton) {
    startEditingDraft(editButton.dataset.editFutureStock);
    return;
  }

  const loadButton = event.target.closest("[data-load-future-stock]");
  if (loadButton) {
    loadDraftToCart(loadButton.dataset.loadFutureStock);
    return;
  }

  const deleteButton = event.target.closest("[data-delete-future-stock]");
  if (deleteButton) {
    deleteDraft(deleteButton.dataset.deleteFutureStock);
    return;
  }

  const whatsapp = event.target.closest("[data-send-future-stock]");
  if (whatsapp) {
    if (whatsapp.getAttribute("aria-disabled") === "true") {
      event.preventDefault();
      dom.status.textContent = "צריך להגדיר מספר וואטסאפ קבוע לפני שליחה.";
      return;
    }
    dom.status.textContent = "ההודעה נפתחה בוואטסאפ. ההזמנה עדיין ממתינה למלאי עתידי ולא נכנסה לדוחות.";
    return;
  }

  const toggleButton = event.target.closest("[data-toggle-future-stock-details]");
  if (!toggleButton) return;
  const card = toggleButton.closest(".order-card");
  const details = card?.querySelector(".order-text-details");
  if (!details) return;
...
function commitDraftToOrders(draftId, options = {}) {
  ...
  const { futureStockOrder: _futureStockOrder, futureStockDate: _futureStockDate, ...orderDraft } = draft;
  const order = {
    ...orderDraft,
    id: `order-${Date.now()}`,
    createdAt,
  ...
  removeFutureStockReminder(draft.id);
  lastPrices = rebuildLastPricesFromOrders(orders);
  saveOrders();
  saveDrafts({ sync: false });
  saveReservations({ sync: false });
...
function startEditingDraft(draftId) {
  ...
  render();
  setActiveTab("cart");
  dom.status.textContent = futureStockOrder
    ? "הזמנת המלאי העתידי נטענה לעריכה. שמירה תעדכן אותה בלבד ולא תכניס אותה להזמנות."
    : "הטיוטה נטענה לעריכה. שמירה תעדכן את הטיוטה הקיימת ולא תכניס אותה להזמנות.";
}
...
function deleteDraft(draftId) {
  ...
  removeFutureStockReminder(draftId);
  ...
  cart = mergeCartLines(
    order.items.map((item) => {
      ...
      return {
        ...
        unitPrice,
        priceSource,
        lineTotal: getCartLineTotal({ ...item, unitPrice }),
      };
    }),
  );
  clearDraftCustomer();
...
function getCartLineTotal(line) {
  if (isBundlePromotionCartLine(line)) {
    const bundleTotal = parsePrice(line.promotionBundleLineTotal);
    if (bundleTotal !== null) return roundMoney(Math.max(0, bundleTotal));
  }
  return roundMoney(parseQuantity(line.quantity) * (Number(line.unitPrice) || 0));
}

function getOrderTotal(items) {
  return roundMoney(items.reduce((sum, item) => sum + getCartLineTotal(item), 0));
}

function getPaidSalesTotal(items) {
  return roundMoney(
    items.reduce(
      (sum, item) => (isReservationOrderItem(item) ? sum : sum + getCartLineTotal(item)),
      0,
    ),
  );
}
...
function readCart() {
  ...
  const normalizedLines = stored
    .map((line) => {
      ...
      const bonusType = normalizeBonusType(line.bonusType);
      const promotionType = normalizePromotionType(line.promotionType);
      const bundleQuantity = parseQuantity(line.promotionBundleQuantity);
      const bundleItemQuantity = parseQuantity(line.promotionBundleItemQuantity);
      const bundleUnitTotal = Math.max(0, parsePrice(line.promotionBundleUnitTotal) ?? 0);
      const isBundle = priceSource === "promotion" && promotionType === "bundle" && cleanString(line.promotionBundleId || line.promotionId);
      return {
        lineKey: createCartLineKey(skuKey, fromReservation, unitPrice, priceSource, bonusType, line.promotionId, promotionType),
        skuKey,
        ...
        promotionType,
        promotionBundleId: isBundle ? cleanString(line.promotionBundleId || line.promotionId) : "",
        promotionBundleQuantity: isBundle ? bundleQuantity : 0,
        promotionBundleItemQuantity: isBundle ? bundleItemQuantity : 0,
        promotionBundlePrice: isBundle ? Math.max(0, parsePrice(line.promotionBundlePrice) ?? 0) : 0,
        promotionBundleUnitTotal: isBundle ? bundleUnitTotal : 0,
        promotionBundleLineTotal: isBundle
          ? Math.max(0, parsePrice(line.promotionBundleLineTotal) ?? roundMoney(bundleUnitTotal * bundleQuantity))
          : 0,
        bonusType,
        quantity: isBundle ? bundleQuantity * bundleItemQuantity : parseQuantity(line.quantity),
        fromReservation,
      };
    })
...
function normalizeOrders(value) {
  ...
  return value
    .map((order) => ({
      ...
      futureStockOrder: Boolean(order.futureStockOrder),
      futureStockDate: normalizeDateInput(order.futureStockDate),
      draftReminderDate: normalizeDateInput(order.draftReminderDate),
      items: Array.isArray(order.items)
        ? order.items.map((line) => ({
            ...
            promotionId: cleanString(line.promotionId),
            promotionName: cleanString(line.promotionName),
            promotionType: normalizePromotionType(line.promotionType),
            promotionBundleId: cleanString(line.promotionBundleId || line.promotionId),
            promotionBundleQuantity: parseQuantity(line.promotionBundleQuantity),
            promotionBundleItemQuantity: parseQuantity(line.promotionBundleItemQuantity),
            promotionBundlePrice: Math.max(0, parsePrice(line.promotionBundlePrice) ?? 0),
            promotionBundleUnitTotal: Math.max(0, parsePrice(line.promotionBundleUnitTotal) ?? 0),
            promotionBundleLineTotal: Math.max(0, parsePrice(line.promotionBundleLineTotal) ?? 0),
...
function normalizePromotions(value) {
  ...
  return value
    .map((item, index) => {
      ...
      const type = normalizePromotionType(item.type);
      return {
        id,
        name,
        description: cleanString(item.description),
        type,
        bundlePrice: type === "bundle" ? Math.max(0, parsePrice(item.bundlePrice) ?? 0) : 0,
